...
class UnifiedBottleDetection:
    def __init__(self):
        ...
        self.operation_count = 0  # Compteur d'opérations validées (bouteilles acceptées)
        self.ready = True  # Indique si le système est prêt pour une nouvelle opération (après validation/rejet)
        ...
        self.cap_detection_padding = 20  # Pixels de padding autour bbox bouteille pour détection bouchon
        ...
        # === BROYAGE tracking ===
        self.broyage_start_time = None    # temps d'envoi de ACC
        self.broyage_finished = False     # True quand Arduino répond "Finished"
        self.broyage_duration_ms = None   # durée en ms
    ...
    def process_segmentation(self, frame, bottle_boxes=None):
        ...
        for box_info in bottle_boxes:
            ...
            if track_id is not None:
                ...
                smoothed_remplie = sum(self.fill_history[track_id]) >= 7  # REMPLIE dès 7/15, VIDE seulement si 9/15
            else:
            ...
            if not self.global_label_locked:
                ...
                if len(self.global_fill_history) >= 15:
                    global_vote = sum(self.global_fill_history) >= 7  # REMPLIE dès 7/15, VIDE seulement si 9/15
        ...
        while True:
            try:
                if self.arduino_serial is None or not self.arduino_serial.is_open:
                    break
                feedback = self.arduino_serial.readline().decode(errors="ignore").strip()
                if feedback:
                    print(f"Feedback Arduino: {feedback}")
                if feedback == "Finished":
                    if self.broyage_start_time is not None:
                        self.broyage_duration_ms = round((time.time() - self.broyage_start_time) * 1000)
                    self.broyage_finished = True
                    self.ready = True
                    print("Arduino ready for next operation")
                    break
            except Exception:
                break
    ...
    def _print_bottle_validity_status(self, current_ids, current_cap_has_cap):
        ...
        for tid in sorted(current_ids):
            ...
            # Enregistrer UNE ligne par bouteille dans le CSV avec labels verrouillés
            self.save_bottle_to_csv(tid, fill_label, cap_locked)
            if is_valid:
                if self.operation_count == 0 or self.ready:
                    self.arduino_serial.write(b'ACC\n')
                    print("ACC sent")

                    self.broyage_start_time = time.time()
                    self.broyage_finished = False
                    self.broyage_duration_ms = None
                    self.ready = False
                    self.operation_count += 1

                    threading.Thread(target=self.wait_for_finished, daemon=True).start()
            else:
                self.arduino_serial.write(b'REJ\n')
                print("REJ sent")
    ...
    def start_realtime_logging(self):
        ...
        csv_dir = r'G:\Mon Drive\csv file robotique'
        os.makedirs(csv_dir, exist_ok=True)
        csv_filename = os.path.join(csv_dir, 'detection_unifie.csv')
        file_exists = os.path.isfile(csv_filename) and os.path.getsize(csv_filename) > 0
        self.realtime_data_file = open(csv_filename, 'a', newline='', encoding='utf-8')
    ...
    def save_realtime_data(self, bottle_exists, num_bottles, is_filled, has_cap):
        """Sauvegarde le statut temps-réel dans le fichier JSON pour l'API."""
        ts = datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-3]
        data = {
            "timestamp": ts,
            "bouteille_existe": "OUI" if bottle_exists else "NON",
            "nombre_bouteilles": num_bottles,
            "bouteille_remplie": "OUI" if is_filled else "NON",
            "bouteille_avec_bouchon": "OUI" if has_cap else "NON",
            "broyage_finished": self.broyage_finished,
            "broyage_duration_ms": self.broyage_duration_ms
        }
        try:
            with open(self.realtime_json_file, 'w', encoding='utf-8') as f:
                json.dump(data, f, ensure_ascii=False, indent=2)
        except Exception as e:
            print(f"[WARN] save_realtime_data JSON: {e}")

    def save_bottle_to_csv(self, track_id, fill_label, cap_locked):
        """Enregistre UNE ligne par bouteille (ID) avec labels verrouillés."""
        if self.realtime_csv_writer is None:
            return
        ts = datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-3]
        is_filled = (fill_label == "REMPLIE")
        statut = "ACCEPTEE" if (not is_filled and not cap_locked) else "REJETEE"
        try:
            self.realtime_csv_writer.writerow([
                ts,
                track_id,
                fill_label,
                "OUI" if cap_locked else "NON",
                statut
            ])
            self.realtime_data_file.flush()
        except Exception as e:
            print(f"[WARN] save_bottle_to_csv: {e}")
        ...
        while True:
          try:
            if not self.paused:
                ...
                # Déterminer le statut remplie/vide depuis le label global verrouillé
                _is_filled = (self.global_locked_label == "REMPLIE") if self.global_label_locked else (water_pct > 0)
                _has_cap = current_cap_has_cap if current_cap_has_cap is not None else False
                _num = 1 if bottle_detected else 0

                # Sauvegarder le statut temps-réel pour l'API (toutes les frames)
                self.save_realtime_data(bottle_detected, _num, _is_filled, _has_cap)

                # Envoyer à Arduino (toutes les 5 secondes) — logique originale inchangée
